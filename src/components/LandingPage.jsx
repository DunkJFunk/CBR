...
const LandingPage = () => {
    ...
    return (
        <div className="w-screen z-10 relative max-md:bg-owhite max-md:h-screen overflow-hidden">
            ...
            <div className="absolute left-60 top-1/3 font-montserrat w-1/2 text-slate-900 flex-row text-left max-md:left-10 max-md:top-48 max-sm:top-28 max-md:w-full max-lg:relative">
                <h1 className="text-7xl font-cinzel font-bold w-3/4 max-sm:hidden">RESTORATION DONE THE RIGHT WAY</h1>
                <p className="text-4xl font-bold mt-4 w-4/5 max-sm:block max-md:text-3xl">
                ...
                <button onClick={handleContact} className="py-4 px-4 bg-slate-900 rounded text-white mt-10 text-2xl max-sm:px-8 hidden max-sm:block">Contact Us</button>
            </div>
        </div>
    );
}
